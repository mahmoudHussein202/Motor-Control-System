...
char stateToString[MAX_SysState_Count][30] = {"Fault State.\r\n", "Communication Lost.\r\n" , "Motor Overheated.\r\n" , "Normal.\r\n"};
Packet PacketToPc ; 
...
ReturnState APP_SendSystemState(void)
{
  ...
  UART_Send_string("Average Temperature : ");
...
ReturnState DTC_Handle(void)
{
  ...
  if(Glob_CommunicationLost_Counter >= DTC_SAVE_AT_REPEATATION)
  {
    // SAVED IN EEPROM AT ADDRESS DTC_COMM_LOST
		while(mainIRQs[IRQ_COMM_LOST]!=0){/* waiting for APP_InterruptBased_GetCANmsg to interrupt the system and reset irq com.lost flag*/}
    retVal = E_OK ; 
  }
  else
  {
...
{
    uint32_t data = 0 ; 
    CAN_xSendRemoteFrame(ECU2_KEEP_ALIVE_ID);
    mainIRQs[IRQ_ALIVE_ACK] =  1 ; 
    // CAN_xReceive(ECU2_KEEP_ALIVE_ID , &data);
}
/**
...
void receving_temp_ECU2( uint32_t  getTemperature , uint32_t  getKnownVoltage)
{
...
********************************************************
/**
 * @brief لسه هعمل فانكشن تكولها لما يحصل انتربت 
 * 
 */
